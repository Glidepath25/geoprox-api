...
def compute_outcome(summary_bins: Dict[str, Dict[str, int]]) -> str:
    ...
    if has25:
        return "MEDIUM"
    # if anything in 25–100m OR nothing at all within 100m
    return "LOW"
...
# Categories → list of OSM filters (each filter is either `key` or `key=value` or `key=*`)
# Keep names aligned with your GUI / client.
OSM_FILTERS: Dict[str, List[str]] = {
    "manufacturing": [
        "industrial", "man_made=works", "landuse=industrial", "factory=*"
    ],
    "gas_holding": [
        "man_made=gasometer", "storage=tank", "gas=storage"
    ],
    "mines": [
        "landuse=quarry", "man_made=mineshaft", "mine=*", "quarry=*"
    ],
    "petrol_stations": [
        "amenity=fuel"
    ],
    "sewage_treatment": [
        "man_made=wastewater_plant", "water=wastewater"
    ],
    "substations": [
        "power=substation"
    ],
    "landfills": [
        "landuse=landfill", "amenity=waste_disposal"
    ],
    "scrapyards": [
        "landuse=scrap_yard", "amenity=scrapyard"
    ],
    "waste_disposal": [
        "amenity=recycling", "amenity=waste_transfer_station", "amenity=waste_disposal"
    ],
}
...
def parse_latlon(raw: str) -> Tuple[float, float]:
    """
    Extract first two floats from a string (robust to spaces/zero-width chars).
    """
...
def geocode_location_flex(loc: str, w3w_key: Optional[str]) -> Tuple[float, float, str]:
    ...
    if loc.startswith("///"):
        ...
        return float(lat), float(lon), f"{words} → {lat:.6f}, {lon:.6f}"
...
def _ovf(token: str) -> str:
    # token like "key", "key=value", or "key=*"
    if "=" in token:
        k, v = token.split("=", 1)
        k = k.strip(); v = v.strip()
        if v == "*":
...
def summarise_by_bins(df: pd.DataFrame, origin: Tuple[float,float]) -> Dict[str, Dict[str, int]]:
    lat0, lon0 = origin
    bins = {"<10m":0, "10–25m":0, "25–100m":0, ">100m / not found":0}
    out: Dict[str, Dict[str, int]] = {}
    if df.empty:
        # still return all categories with zeros
        for label in [
            "Industrial / Manufacturing","Gas holder stations","Mining (coal, metalliferous)",
            "Petrol stations / Garages","Sewage Treatment Works","Sub-Stations",
            "Waste Site – Landfill & Treatment / Disposal","Waste Site – Scrapyard / Metal Recycling",
            "Waste Site – Other"
        ]:
            out[label] = dict(bins)
        return out
    ...
    def dist_m(row):
        try: return _haversine_m(lat0, lon0, float(row["lat"]), float(row["lon"]))
        except: return float("inf")

    df = df.copy()
    df["distance_m"] = df.apply(dist_m, axis=1)

    # Category mapping (display -> predicate)
    cat_map = {
        "Industrial / Manufacturing": lambda t: ("industrial" in t) or (t.get("man_made")=="works") or (t.get("landuse")=="industrial") or ("factory" in t),
        "Gas holder stations":        lambda t: (t.get("man_made")=="gasometer") or (t.get("storage")=="tank") or (t.get("gas")=="storage"),
        "Mining (coal, metalliferous)": lambda t: (t.get("landuse")=="quarry") or ("quarry" in t) or (t.get("man_made")=="mineshaft") or ("mine" in t),
        "Petrol stations / Garages":  lambda t: (t.get("amenity")=="fuel"),
        "Sewage Treatment Works":     lambda t: (t.get("man_made")=="wastewater_plant") or (t.get("water")=="wastewater"),
        "Sub-Stations":               lambda t: (t.get("power")=="substation"),
        "Waste Site – Landfill & Treatment / Disposal": lambda t: (t.get("landuse")=="landfill") or (t.get("amenity") in {"waste_disposal","waste_transfer_station"}),
        "Waste Site – Scrapyard / Metal Recycling":     lambda t: (t.get("landuse")=="scrap_yard") or (t.get("amenity")=="scrapyard"),
        "Waste Site – Other":         lambda t: (t.get("amenity") in {"recycling"}) and (t.get("landuse")!="scrap_yard"),
    }

    for disp, pred in cat_map.items():
        dfi = df[df["tags"].apply(lambda t: pred(t or {}))]
        b = {"<10m":0, "10–25m":0, "25–100m":0, ">100m / not found":0}
        if not dfi.empty:
            ...
            b["<10m"] = int(d10); b["10–25m"] = int(d25); b["25–100m"] = int(d100); b[">100m / not found"] = int(rest)
        out[disp] = b
    return out
...
def build_details_rows(df: pd.DataFrame, origin: Tuple[float,float]) -> List[Tuple[Any, ...]]:
    ...
    df = df.copy()
    df["distance_m"] = df.apply(dist_m, axis=1)
    df = df[df["distance_m"] <= 100].sort_values("distance_m")
    rows = []
...
def _display_category(tags: Dict[str, Any]) -> str:
    t = tags or {}
    if (t.get("amenity")=="fuel"): return "Industrial / Manufacturing" if False else "Petrol stations / Garages"
    if (t.get("power")=="substation"): return "Sub-Stations"
    if (t.get("man_made")=="wastewater_plant") or (t.get("water")=="wastewater"): return "Sewage Treatment Works"
    if (t.get("landuse")=="quarry") or ("quarry" in t) or (t.get("man_made")=="mineshaft"): return "Mining (coal, metalliferous)"
    if (t.get("landuse")=="industrial") or (t.get("man_made")=="works") or ("factory" in t): return "Industrial / Manufacturing"
    if (t.get("landuse")=="landfill") or (t.get("amenity") in {"waste_disposal","waste_transfer_station"}): return "Waste Site – Landfill & Treatment / Disposal"
    if (t.get("landuse")=="scrap_yard") or (t.get("amenity")=="scrapyard"): return "Waste Site – Scrapyard / Metal Recycling"
    if (t.get("amenity")=="recycling"): return "Waste Site – Other"
    if (t.get("man_made")=="gasometer") or (t.get("storage")=="tank"): return "Gas holder stations"
    return "Other"
...
def generate_pdf_summary(
    ...
    # outcome color band
    color_map = {
        "HIGH": colors.HexColor("#c62828"),    # red
        "MEDIUM": colors.HexColor("#ef6c00"),  # orange
    ...
    info_tbl.setStyle(TableStyle([
        # Outcome badge cell background (row 0, col 1)
    ...
    # ---- Summary table with ticks (unchanged logic you now have) ----
    ...
    def to_checks(bins: Dict[str, int]) -> List[str]:
        ...
        no = not (has10 or has25 or has100)  # “No” = nothing within 100m
    ...
    # ---- Details table (≤100 m), wrapped & sized for A4 (as you have now) ----
    ...
    det_tbl = Table(det_data, repeatRows=1,
                    colWidths=[24*mm, 34*mm, 46*mm, 20*mm, 20*mm, 46*mm])
...
def run_geoprox_search(
    ...
    VALID_CATS = set(OSM_FILTERS.keys())
    categories = [c for c in (categories or []) if c in VALID_CATS]
    if not categories:
    ...
    safe_permit = "".join(ch if ch.isalnum() or ch in "-_" else "_" for ch in permit)[:60]
    ...
    details_rows_json = [
        {"distance_m": int(r[0]), "category": r[1], "name": r[2], "lat": r[3], "lon": r[4], "address": r[5]}
        for r in details
    ]
    ...
    if bucket:
        try:
            ...
            pdf_url  = s3.generate_presigned_url(
                "get_object", Params={"Bucket": bucket, "Key": pdf_key}, ExpiresIn=86400
            )
            html_url = s3.generate_presigned_url(
                "get_object", Params={"Bucket": bucket, "Key": html_key}, ExpiresIn=86400
            )
